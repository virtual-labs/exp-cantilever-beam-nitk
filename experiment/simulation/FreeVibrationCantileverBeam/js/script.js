...
function varupdate() {
  ...
  document.getElementById("massSystem").title =
    "M =" +
    m.toFixed(4) +
    " kg" +
    "\n" +
...
function plotgraph() {
  ...
  ctx.lineWidth = 1;
  while (i < 300) {
    ctx.lineTo(i + 20, 225 - (0.9 * actdisplace(0.003 * i)) / 5);
    ctx.moveTo(i + 20, 225 - (0.9 * actdisplace(0.003 * i)) / 5);
    i += 0.01;
  }
  ctx.stroke();
  ctx.beginPath();
  ...
  ctx.stroke();
  ctx.font = "12px Arial";
  ctx.fillText("\u03C9<sub>d= " + wd.toFixed(3) + "rad/s", 430, 370);
}
