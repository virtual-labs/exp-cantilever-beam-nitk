...
function varinit() {
  ...
  $("#slidergraph").draggable({
    axis: "x",
    containment: "#constraintbox",
    drag: function () {
      printcordinates($("#slidergraph").position().left);
    },
  });
...
function varupdate() {
  ...
  //   "Note: Hover on the graph to display the Displacement and Time";
...
function generateGraph(x, y) {
  ...
  graphctx1.restore();
  graphctx1.fillText("Time", 150, 240);
  ...
  graphctx1.beginPath();
  graphctx1.font = "bold 1.2rem 'Nunito', sans-serif";
  graphctx1.fillStyle = "red";
  graphctx1.fillText("Note:Hover on the graph to display the Displacement and Time", 2, 15);
  ...
  graphctx2.restore();
  graphctx2.fillText("Frequency(rad/s)", 20, 260);
...
function getCoordinates(event) {
  ...
  const clientX =
    event.clientX - offsetX || (event.touches && event.touches[0].clientX);
